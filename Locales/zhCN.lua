...
• 小地图只显示已勾选“显示标记材质”的正式标记和路线临时标记，不显示地点名称；仅显示文字的地点不会出现在小地图上。
...
• 不打开世界地图时，地图区域结果和“所有地图”中非角色当前地图的自建地点会自动打开目标地图；当前地图地点和手动输入的坐标结果只设置原生路径点。
• 世界地图未打开时，右键自建地点搜索结果可选择“打开地图”，跳转后闪烁目标位置。
• 当搜索框输入 22 22，或 22,22 等符合坐标格式的数据时，会自动识别并展示结果；左键定位，右键可复制、加入路线或添加临时标记。
• 点击空搜索框会显示最近 10 条查询词；主面板会显示最近 10 个成功选择的搜索结果。
