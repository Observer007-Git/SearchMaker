...
| 路线队列 | ✅ | ✅ | ✅ | ✅ | 会话级手动队列，支持排序、定位、完成并下一个和临时地图标记；不计算最短路线且不写入存档 |
...
| 数据持久化 | ✅ | ✅ | ✅ | — | 自建地点、备注、设置和使用次数保存在账号级 `SearchMakerDB`；路线和外部缓存不写入存档 |
...
| 路线和导入预览框 | `UI/RouteDialog.lua` / `UI/ImportPreviewDialog.lua` |
