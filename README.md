...
`enUS` 和 `zhCN`。它使用游戏原生路径点定位地点，并可在世界地图和小地图上显示自定义标记。

主要功能：

- 搜索当前地图或全部已保存地点，并可搜索地图名称。
- 保存最近 10 条查询词和最近 10 个已选择的搜索结果，支持快速再次搜索或定位。
...
插件不依赖第三方库。安装并启用 `HandyNotes` 与 `HandyNotes_MapNotes` 后，默认提供
当前地图的外部地点搜索；关闭“开启第三方插件坐标搜索”后，不再初始化或缓存这些数据。
...
| 搜索历史与最近结果 | ✅ | ✅ | ✅ | ✅ | 搜索框获得焦点时显示最近 10 条查询词；主面板显示最近 10 个已成功选择的结果 |
| 地图范围 | 当前浏览地图或角色当前地图 | 全部自建地点 | 仅 `mapID` 完全相同的地图 | 仅当前地图 | 不会把子地图地点自动投影到大陆或上层地图 |
| 原生路径点定位 | ✅ | ✅ | ✅ | ✅ | 坐标输入、自建地点和外部地点均使用游戏原生路径点；全图搜索选择非角色当前地图的自建地点时会先打开目标地图 |
| 定位高亮动画 | ⚠️ | ⚠️ | ✅ | ✅ | 仅在世界地图正显示目标地图时播放约 3 秒高亮；搜索结果悬停只显示动画，不显示额外标记 |
| 持久化地图标记 | ✅ | ⚠️ | ✅ | — | 世界地图显示地点自己的材质和名称选项；小地图只显示已启用材质的自建地点，不显示名称 |
...
| 数据持久化 | ✅ | ✅ | ✅ | — | 自建地点、保存路线、搜索历史、备注、设置和使用次数保存在账号级 `SearchMakerDB`；当前执行路线和外部缓存不写入存档 |
...
- 世界地图未打开时，左键选择“所有地图”中的非当前地图自建地点会自动打开目标地图、设置路径点并闪烁目标位置
...
- 主面板地点与常用地点的木牌左侧显示分组图标或该地点的自定义图标，并使用与搜索结果
  一致的图标边框。
- 主面板“最近搜索结果”保留最近 10 个成功选择的结果，最后选择的排在最前面。
...
- 地点名称默认使用金色；勾选“显示标记文字颜色”的地点会保存自己的颜色。
- “导出/导入”：备份、分享或导入地点；“从密语导入”读取本次登录或 `/reload` 后通过
  角色/战网密语收发的 `SMK|` 文本，不扫描其他频道；所有导入均先预览再确认写入。
- “路线规划”：位于地点比例调整右侧。为当前队列输入名称后可保存，保存路线可通过
  `SMK|R|` 字符串分享和导入；搜索“路线”可重新打开或激活。“自动排序”只调整连续
  的同地图地点，角色所在地图从角色坐标开始，其他地图保留区段首个地点，不跨地图重排。
- “批量删除”位于“新增坐标”右侧；“使用说明”位于“标记设置”右侧。
- 路线中的每个地点可添加特殊临时标记。临时标记不进入地图标记材质池，不写入存档，
  清空或移除路线时仍保留，`/reload` 或退出游戏后消失；已有正式标记不会被覆盖或清除。
- “搜索设置”：调整搜索框 UI 样式、缩放、透明度和快捷键，也可限制搜索框只在世界地图打开时显示，或关闭第三方插件坐标搜索；支持“带头像”“不带头像”和“暴雪原生”三种样式。“不带头像”会按角色阵营使用联盟或部落任务标题背景，并按材质原始比例等比显示。
- “标记设置”：调整地图标记材质大小、文字大小及水平/垂直偏移；是否显示材质和文字由
  每个地点新增/修改面板中的选项单独决定。
- 小地图只显示已勾选“显示标记材质”的自建地点和路线临时标记，不显示地点名称；
  仅勾选“显示标记名称”的地点不会出现在小地图上。
